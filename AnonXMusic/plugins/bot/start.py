...
async def welcome(client, message: Message):
    for member in message.new_chat_members:
        ...
        # ✅ Group join count: Store group ID
        if member.id == app.id:
            ...
            if member.id == app.id:
                ...
                if (
                    ...
                    await app.send_message(
                        LOGGER_ID,
                        f"Group auto-blacklisted due to Myanmar characters.\n"
                        f"Title: {ch.title}\nID: {message.chat.id}",
                    )
                    return await app.leave_chat(message.chat.id)
...
# ✅ BOT LEFT — REMOVE GROUP FROM COUNTER
...
# ✅ COMMAND: /groups  → Show total groups bot added in
@app.on_message(filters.command("groups") & ~BANNED_USERS)
async def show_groups(client, message: Message):
    groups = read_groups()
    await message.reply_text(f"✅ Bot is currently added in **{len(groups)}** groups.")
